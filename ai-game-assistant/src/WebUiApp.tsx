...
const BUTTON_LAYOUT: Array<{ label: string; button: GameButton; kind?: 'accent' }> = [
  { label: 'Up', button: 'UP' },
...
const WebUiApp: React.FC = () => {
  ...
  const draftProviderMeta = PROVIDER_META[settingsDraft.provider] || {
    label: settingsDraft.provider,
    helper: 'This provider is exposed by the backend but does not have frontend-specific guidance.',
  };
  ...
  const handleUploadRom = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      await runTask('upload-rom', async () => apiService.uploadRom(file, settings.emulatorType, settings.launchUiOnLoad), {
  ...
  const handlePressButton = async (button: GameButton) => {
    apiService.setBaseUrl(settings.backendUrl);
    await runTask(`button-${button}`, () => apiService.pressButton(button), {
  ...
  const handleManualAction = async () => {
    ...
    await runTask('manual-action', () => apiService.executeAction(manualAction, frames), {
  ...
  const requestAiAction = async (applyAfter = false) => {
    ...
    apiService.setBaseUrl(settings.backendUrl);
    const result = await runTask('ai-action', () => apiService.requestAiAction({
    ...
    if (applyAfter) {
      await runTask('ai-apply', () => apiService.executeAction(result.action, 1), {
  ...
  const handleApplySuggestion = async () => {
    ...
    apiService.setBaseUrl(settings.backendUrl);
    await runTask('apply-suggestion', () => apiService.executeAction(aiSuggestion.action, 1), {
  ...
  const handleSendChat = async () => {
    ...
    try {
      const response = await runTask('chat', () => apiService.chat({
        message,
        api_name: settings.provider,
        api_key: settings.apiKey || undefined,
        api_endpoint: settings.apiEndpoint || undefined,
  ...
  const handleApplyAgentMode = async () => {
    apiService.setBaseUrl(settings.backendUrl);
    await runTask('agent-mode', () => apiService.setAgentMode({
  ...
  const handleUiControl = async (action: 'launch' | 'stop' | 'restart') => {
    ...
    await runTask(`ui-${action}`, operation, {
  ...
  const handleClearCache = async () => {
    apiService.setBaseUrl(settings.backendUrl);
    await runTask('clear-cache', () => apiService.clearCache(), {
  ...
  const handleReadMemory = async () => {
    ...
    apiService.setBaseUrl(settings.backendUrl);
    const result = await runTask('memory-read', () => apiService.readMemory(address, size, memoryForm.format), {
  ...
  const handleWriteMemory = async () => {
    ...
    apiService.setBaseUrl(settings.backendUrl);
    const result = await runTask('memory-write', () => apiService.writeMemory(address, values), {
      ...
      <header className="topbar">
        ...
        <div className="topbar__actions">
          ...
          <button
            ...
            onClick={() => void runTask('refresh-all', refreshAll, { source: 'backend', silent: true })}
            disabled={busy['refresh-all']}
          >
            <RefreshCw size={16} />
            Refresh
          </button>
              ...
              {settingsDraft.provider === 'openai-compatible' ? (
                <div className="field">
                  ...
                  <input
                    ...
                    placeholder={draftProviderMeta.endpointPlaceholder}
                  />
                </div>
              ) : null}

              <div className="field">
                <label>{draftProviderMeta.keyLabel || 'API key'}</label>
                <input
                  type="password"
                ...
                />
                <small>{draftProviderMeta.helper}</small>
              </div>
