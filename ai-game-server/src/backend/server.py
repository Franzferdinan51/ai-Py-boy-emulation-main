            ...
            while True:
                try:
                    ...
                    if elapsed >= frame_interval:
                        try:
                            # Direct PyBoy calls - no executor overhead
                            try:
                                success = emulator.step('NOOP', 1)
                                pyboy_timeout_count = 0
                            except Exception as e:
                                pyboy_timeout_count += 1
                                logger.warning(f"PyBoy step error (count: {pyboy_timeout_count}): {e}")
                                if pyboy_timeout_count >= max_pyboy_timeouts:
                                    raise TimeoutError(f"Too many PyBoy errors: {pyboy_timeout_count}")
